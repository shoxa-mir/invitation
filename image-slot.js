...
(() => {
  ...
  // sidecar enormous. The write bridge HARD-REJECTS a combined sidecar
  // write past ~2.1MB ("content too large") — confirmed empirically, not
  // documented — and this page can have 7+ slots sharing that one budget.
  // Lean on quality (which compresses much more aggressively than
  // resolution for photographic webp) rather than shrinking pixel
  // dimensions — a full-bleed hero/gallery slot displayed at 1200px+ reads
  // as visibly soft if capped much below that, whereas dropping quality
  // from ~0.9 to ~0.72 cuts bytes roughly in half with much less visible
  // softening.
  const MAX_DIM = 1800;
  ...
  const subs = new Set();
  // Elements currently connected, separate from `subs` (bound render
  // callbacks, not element refs) so a failed save can flash a visible
  // error on every slot on the page -- a console.warn alone is invisible
  // to the person who just dropped a photo and saw it vanish.
  ...
  function save() {
    ...
    Promise.resolve(w(STATE_FILE, JSON.stringify(slots)))
      .catch((err) => {
        console.warn('<image-slot> sidecar save failed -- dropped image(s) will not persist across reload:', err);
        const msg = 'Save failed (' + ((err && err.message) || 'images too large') + ') -- try a smaller/fewer photos';
        for (const el of liveElements) { if (el._setError) el._setError(msg); }
      })
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      // Floor of 650px so small slots (avatar-sized circles, etc.) don't get
      // hard-downsampled to 2x their tiny rendered box — still capped by
      // MAX_DIM overall.
      const cap = Math.min(MAX_DIM, Math.max(800, Math.round(targetW * 2)) || MAX_DIM);
